...
# Loop through each deacde and save separate LDA results. 
...
for (i in 1:(length(lda_models) - 1)) {
  ...
  cor_matrix <- generate_heatmap(model1, model2, correlation_threshold = 0.9)
...
# Function to print the ordered rows for each topic with high correlation
print_ordered_rows <- function(phi1_union, phi2_union, cor_matrix, correlation_threshold = 0.9) {
  # Find indices where correlation is higher than the threshold
  high_corr_indices <- which(cor_matrix > correlation_threshold & !is.na(cor_matrix), arr.ind = TRUE)
  ...
  for (i in seq_len(nrow(high_corr_indices))) {
    ...
    cat(paste("Model 2 - Topic", model2_topic), "\n")
    phi2_result_row <- orderBasedOnRow(phi2_union, model2_topic)
    #print(phi2_result_row)

    # Convert result rows to long format
    phi1_result_long <- phi1_result_row %>%
      tidyr::pivot_longer(everything(), names_to = paste0("terms_",rownames(phi1_result_row)), values_to = "probability_1")

    phi2_result_long <- phi2_result_row %>%
      tidyr::pivot_longer(everything(), names_to = paste0("terms_",rownames(phi2_result_row)), values_to = "probability_2")
  ...
  # Combine all results into a single dataframe
  final_result <- do.call(bind_cols, result_list)
...
for (i in 1:(length(lda_models) - 1)) {
  ...
  if (any(cor_matrix > 0.9, na.rm = TRUE)) {
    ...
    # Call the print_ordered_rows function and pass relevant parameters
    final_result <- print_ordered_rows(phi1_result, phi2_result, cor_matrix)
    print(final_result)
  }
}
